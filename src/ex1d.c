...
We are interested in finding the `easiest' equivalent presentation.  We do
this by looking for easy enumerations over a 1-generator subgroup; we know
that this can be done with a total of less than 10M cosets.  We preserve 
the ordering of the relators, but consider all inverses & cyclic shifts 
(ie, (2x25)^3 = 125000 presentations) and all subgroups (ie, "r", "s" &
"t").  We'll run in "sims:3" mode (which seems to be always close to best).
We enforce a hard limit on t by disallowing compaction.
...
int main(void)
  ...
  /* Cycled/inverted runs, with no messaging; one run matches run 0. */
