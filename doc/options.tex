...
{\ACE} package to `false', and is the default action that {\ACE} takes
for options that are unknown to the {\ACE} package  (but  may  be  new
options provided in a new version of the {\ACE} binary). If  the  user
sets `ACEIgnoreUnknownDefault := true', then  for  any  function  call
requiring {\ACE} for which the user does not want options  unknown  to
the package to be  ignored,  the  user  pass  among  the  option  list
`aceignoreunknown   :=   false'   (see   "option   aceignoreunknown"),
