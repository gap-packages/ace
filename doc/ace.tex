...
obtained with {\GAP}~4.2. {\ACE} can be used through this interface in
a variety of ways:
...
iostreams.  If  you  encounter  this  problem,  consider   using   the
non-interactive `ACEStats' (see~"ACEStats"), which only uses files, to
first  test  whether  the  coset  enumeration  will  succeed.  Another
alternative, if one is looping over several possibilities of  <fgens>,
<rels> and <sgens>, is to just use one interactive {\ACE} process, and
re-use the process continually in something like the following manner:
...
See~"ACEStart", for the two special forms of  `ACEStart'  used  above,
and see~"ACEStats!interactive" and~"ACECosetTable!interactive" for the
interactive usages of `ACEStats' and `ACECosetTable'.
...
The {\ACE} Package is compatible with {\GAP} 4.2, but has  been  built
for use with {\GAP} 4.3. One important change in  {\GAP}  from  {\GAP}
4.2 to {\GAP} 4.3 that has relevance for {\ACE} Package users  is  the
change of the default standard for the numbering of cosets in a  coset
table (see Section~"Coset Table Standardisation Schemes").
...
`ACECosetTable') invoke iostreams (previously files were used,  as  is
still the case with the non-interactive `ACEStats').
