...
already started interactive {\ACE} process, have a form where the first  argument  is
...
process). We will thus  commonly  say  that  \lq{}for  the  <i>th  (or
default) interactive {ACE} process'  a  certain  function  performs  a
given action. In each case, it is an error, if <i> is not the index of
an  active  interactive  process,  or  there  are  no  current  active
...
\lq{}experimentation'   {\ACE}   modes   and    describe    them    in
...
internal version of `ACEModes'  is  run  to  determine  which  of  the
general {\ACE} modes (see Section~"General ACE Modes")  `ACEContinue',
...
these that is allowed is executed, to ensure the resultant  statistics
are correct for the current options.
...
lenlex"). For more details, see the discussion for the non-interactive
version              of              `IsACEGeneratorsInPreferredOrder'
("IsACEGeneratorsInPreferredOrderNI"),  which  is  called   with   two
arguments.
