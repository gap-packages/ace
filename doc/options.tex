...
However, `FlushOptionsStack' does not wipe  out  the  options  already
passed  to  an  *interactive*  {\ACE}  process.   We   have   provided
`GetACEOptions' (see~"GetACEOptions") to keep track  of  options  that
the {\ACE} binary process still considers active, which may or may not
be  still  on   the   `OptionsStack'.   There   is   the   interactive
`SetACEOptions' (see~"SetACEOptions") to change such options,  or,  of
course, you can always elect to use `ACEQuit' (see~"ACEQuit") and then
start a new interactive {\ACE} process.

Finally, there will be situations where an {\ACE}  interface  function
...
{\ACE} share package to `false', and is the default action that {\ACE}
takes for options that are unknown to the {\ACE}  share  package  (but
may be new options provided in a new version of the {\ACE} binary). If
the user sets `ACEIgnoreUnknownDefault := true', then for any function
call requiring {\ACE} for which the user does not want options unknown
to the share package to be ignored, the user  pass  among  the  option
list `aceignoreunknown  :=  false'  (see  "option  aceignoreunknown"),
explicitly (thus reversing the usual behaviour).
...
which you intend to use. Omitting  the  `aceignoreunknown'  option  is
equivalent to setting it to  the  value  of  `ACEIgnoreUnknownDefault'
(see "ACEIgnoreUnknownDefault").
