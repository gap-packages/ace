...
:=500').  The  value  of  `table',  i.e.~the   {\GAP}   coset   table,
immediately follows the last {\ACE} message (\lq{}`\#I '')  line,  but
...
`G' are not accessible, being \lq{}local' to `ACEExample').
...
The return value on the last line is an  \lq{}index'  that  identifies
the interactive process; we use this \lq{}index' with  functions  that
need  to  interact  with  the  interactive  {\ACE}  process;  we   now
demonstrate this with the interactive version of `ACEStats':
