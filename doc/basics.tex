...
standardisation scheme}} scheme, of  Charles  Sims~\cite{Sim94}.  This
...
generators; it is known as `semilenlex'{\undoquotes\atindex{semilenlex
standardisation scheme} {@`semilenlex' standardisation scheme}}  since
...
*loop*\index{loop} or a *pass*\index{pass} through  such  a  loop.  We
...
where each *state*\index{state (machine)} is  a  value  of  the  coset
...
loop; if the option `loop' (see~"option loop") is set  to  a  positive
integer, {\ACE} makes an early return when the  loop  count  hits  the
value of `loop'.
