...
Observe, that one *can* add additional options to those  already  used
in an example (but since the example's options are \lq{}seen'  *after*
any added options, one can't over-ride any of  the  example's  options
\dots\ not directly, anyhow).  What  one  *can*  do,  is  to  force  a
`break'-loop{\undoquotes\atindex{break-loop}{@`break'-loop}}, and then
use the `SetACEOptions' function that becomes available to reset *any*
options as one likes (see Section~"Fun with ACEExample  Example").  On
the other hand, for `<ACEfunc> = StartACE', one doesn't need to  force
a `break'-loop; since the {\ACE} session is still alive,  new  options
can be added (and  the  enumeration  re-started)  via  the  `StartACE'
function.
...
that prior to calling `StartACE' that the `OptionsStack' be empty.
...
`ACEStats',  `ACECosetTable'  or  `StartACE')  is  given  no  strategy
...
`StartACE') is essentially  to  restore  as  much  as  is  possible  a
...
the `known' field is set `true', or <optname> otherwise.
...
Interactively, use `StartACE' (see~"StartACE").
...
Interactively, use `QuitACE' (see~"QuitACE").
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\Section{Fun with ACEExample Example}
...
#I     or ACEStats, or StartACE. The 2nd argument is *not* quoted.
...
Now let's observe that we can add some new options;  but  first  we'll
reduce the output a bit by setting the `InfoLevel' of `InfoACE' to  1,
use the default function `ACECosetTableFromGensAndRels'  and  so  it's
like our first encounter with F(2,7) we'll add the subgroup  generator
`c' via `sg := [c]' (see "Option `sg'"); and while we're at it,  we'll
force     a     `break'-loop{\undoquotes     \atindex     {break-loop}
{@`break'-loop}}, to demonstrate how we can over-ride any options  the
example already sets.

\beginexample
gap> SetInfoACELevel(1);                               
gap> ACEExample("F27-purec" : max := 2, sg := [c]);
#I  # ACEExample "F27-purec" : enumeration of cosets of H in G,
#I  # where G = F(2,7) = C_29, H = Id, using purec strategy.
#I  #
#I  # F, G, a, b, c, d, e, x, y are local to ACEExample
...
Entering break read-eval-print loop, you can 'quit;' to quit to outer loop,
or you can return to continue
brk> # Observe how `max := 2' which forced the break, was seen first
brk> # ... so if we had entered the `hlt' option it would just have
brk> # been over-ridden by `purec', but we are in the break-loop now
brk> # ... so we are free to change to `hlt' now:
brk> SetACEOptions(: hlt := false, hlt, echo := 2, max := 0);
...
Observe the \lq{}`Other options set via ACE defaults'' list of options
that has resulted from having the  `echo  :=  2'  option  (see~"Option
`echo'"). Observe, also, that `hlt' is nowhere near as good, here,  as
`purec' (check with Section~"Using ACE Directly to  Generate  a  Coset
Table").

If we had passed `hlt' initially,  and  then  tried  from  within  the
`break'-loop to over-ride `purec', by the passing `hlt',  it  wouldn't
have worked, because the way {\GAP}'s `OptionsStack'  works  it  would
have remained in the same position in the  stack  (and  `purec'  would
have again over-ridden it); but we can trick {\GAP}! What we can do is
use the fact that {\ACE} interface options can be uppercase, lowercase
or even mixed case. So, if we pass `HLT' via  `SetACEOptions'  in  the
`break'-loop, {\GAP} won't know that it's the same as `hlt'  and  push
it onto the end of the stack, in which case `hlt' will be  over-ridden
by `purec' which will in turn be over-ridden by `HLT'.

Of course, running `ACEExample' with  `StartACE'  as  second  argument
opens up far more flexibility. Try it! Have fun!  Play  with  as  many
options as you can. (For the moment you will sometimes need  the  hint
regarding changing case to enforce the order of options you want, even
with `StartACE'; in the next  revision  however  this  *bug*  will  be
fixed.)
