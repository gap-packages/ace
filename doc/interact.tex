...
defined) are tracked, and each time a new  ``record''  is  found,  the
...
(see~"ACECosetTable!non-interactive"), local versions  of  the  second
...
(see~"ACECosetTable!non-interactive"), albeit  interactively,  on  the
<i>th (or default) process  started  by  `ACEStart'.  If  options  are
...
ensure the resultant table is correct for the current options.

\>ACEStats( <i> [:<options>] )!{interactive} F
\>ACEStats( [:<options>] )!{interactive} F

perform  the  same  function  as   `ACEStats'   on   three   arguments
(see~"ACEStats!non-interactive"), albeit interactively, on  the  <i>th
...
("IsACEGeneratorsInPreferredOrder!non-interactive"), which  is  called
with two arguments.
