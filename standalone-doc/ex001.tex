...
We can match Sims' values for \ttt{sims:1} \amp \ttt{sims:3} (the R style 
  \amp R* style strategies without `mendelsohn') and for \ttt{sims:9} 
  (the C style strategy), but sometimes may not do so for the strategies
  that have `mendelsohn' on (\ttt{sims:5} \amp \ttt{7}); this makes sense,
...
The input and output files for \cite[Example 5.3]{Sim94}, $k=8$:
