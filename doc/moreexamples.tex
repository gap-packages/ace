...
Here we demonstrate the various `sims' strategies (see~"option sims"),
and see if we can duplicate the `m' (maximum active coset numbers) and
`t'  (total  coset  numbers  defined)  values  (see  the  input   file
`ex001.in'). The  ability  to  do  so  gives  our  confidence  in  the
correctness of {\ACE} a large boost. We work with the formal  inverses
...
*Do I need to ensure `lenlex' option is in force to ensure this? ...
 or can I just mention it and leave it to the user?*

We may also have to use the `asis' flag, to force the column order (by
entering involutions as `xx') and to preserve the relator ordering.

We match Sims' values for R style and R\* style (`sims := 1' and `sims
:= 3') and C style (`sims :=  9'),  but  may  not  do  so  if  we  use
Mendelsohn (`sims := 5' and `sims := 7'); this makes sense, since  the
order of processing cycled relators is not specified by Sims.

*THE FOLLOWING EXAMPLES ARE in ``RAW'' ACE AND NEED TO BE TRANSLATED
 TO THE EQUIVALENT IN GAP*
