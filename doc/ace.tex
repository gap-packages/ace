...
Interactively, use `ACENormalClosure' (see~"ACENormalClosure").
...
Interactively, use `ACEStabilisingCosets' (see~"ACEStabilisingCosets").
...
`InfoACE' or `InfoWarning' level 1.
...
enumeration via `ACEStart' (see~"ACEStart").
...
For the <i>th (or default) interactive  {\ACE}  process,  applies  any
<options> and then \lq{}continues' the current  enumeration,  building
...
For the <i>th (or default) interactive  {\ACE}  process,  applies  any
<options> and then \lq{}redoes' the current enumeration; any  existing
...
interactive {\ACE} process. If no generators were saved an empty  list
is  returned  and  an  `Info'  message  is  emitted  at  `InfoACE'  or
`InfoWarning' level 1. Essentially, `ACEGroupGenerators(<i>)'  returns
`ACEData.io[<i>].args.fgens' if bound, or an empty list, otherwise.

\>ACERelators( <i> ) F
\>ACERelators() F

return the {\GAP} relators, of  the  <i>th  (or  default)  interactive
{\ACE} process. If no relators were saved an empty  list  is  returned
and an `Info' message is emitted at `InfoACE' or  `InfoWarning'  level
1. Essentially, `ACERelators(<i>)' returns `ACEData.io[<i>].args.rels'
if bound, or an empty list, otherwise.

\>ACESubgroupGenerators( <i> ) F
\>ACESubgroupGenerators() F

return the {\GAP} subgroup  generators,  of  the  <i>th  (or  default)
interactive {\ACE} process. If no subgroup generators  were  saved  an
empty list is returned and an `Info' message is emitted  at  `InfoACE'
or `InfoWarning' level  1.  Essentially,  `ACESubgroupGenerators(<i>)'
returns `ACEData.io[<i>].args.sgens'  if  bound,  or  an  empty  list,
otherwise.
...
returns, for the <i>th (or default) process started by  `ACEStart',  a
...
The compaction routine is called automatically when any of `ACECycles'
(see~"ACECycles"),    `ACENormalClosure'     (see~"ACENormalClosure"),
`ACEDisplayTable'              (see~"ACEDisplayTable")              or
`ACEStandardCosetNumbering'    (see~"ACEStandardCosetNumbering")    is
invoked.
...
table of <i>th (or default)  interactive  {\ACE}  process  started  by
...
\>ACEAddRelators( <i>, <relatorlist> ) F
\>ACEAddRelators( <relatorlist> ) F

add, for the <i>th (or default) interactive {\ACE} process started  by
`ACEStart', the words  in  the  list  <relatorlist>  to  any  relators
already  present,  and  automatically  invokes  either  `ACERedo'   or
`ACEStart'. Note that, {\ACE} chooses an \lq{}optimal' ordering of the
resultant relator list, unless the `asis' option (see~"Option `asis'")
has been set to 1; don't assume, unless `asis  :=  1',  that  the  new
relators have been appended in user-provided order to  the  previously
existing relator list.
...
automatically invokes either `ACERedo' or `ACEStart'. The new subgroup
generators are simply appended by  {\ACE}  to  the  existing  list  of
subgroup generators in user-provided order.

\>ACEDeleteRelators( <i>, <list> ) F
\>ACEDeleteRelators( <list> ) F

delete, for the <i>th (or default) interactive {\ACE} process  started
by `ACEStart', from the current relators those indexed by the integers
in <list>, which must be a strictly increasing  sequence  of  positive
integers, and automatically  invoke  `ACEStart'.  Use  `ACEParameters'
(see~"ACEParameters") to determine the current relator list.

\>`dr:=<list>'{Option `dr'}&
`D'elete relators; <list> must be a list of positive integers.

\>ACEDeleteSubgroupGenerators( <i>, <wordlist> ) F
\>ACEDeleteSubgroupGenerators( <wordlist> ) F

delete, for the <i>th (or default) interactive {\ACE} process  started
by `ACEStart', from the current subgroup generators those  indexed  by
the integers in <list>, which must be a strictly  increasing  sequence
of  positive  integers,  and  automatically  invoke  `ACEStart'.   Use
`ACEParameters'  (see~"ACEParameters")  to   determine   the   current
subgroup generator list.

\>ACECosetCoincidence( <i>, <int> ) F
\>ACECosetCoincidence( <int> ) F

print, for the <i>th (or default) interactive {\ACE}  process  started
by `ACEStart', out the representative of coset <int>, and  add  it  to
the subgroup generators; i.e., equates this coset with  coset  1,  the
subgroup. `ACERedo' is automatically invoked.

\>ACERandomCoincidences( <i>, [<index>] ) F
\>ACERandomCoincidences( [<index>] ) F
\>ACERandomCoincidences( <i>, [<index>, <attempts>] ) F
\>ACERandomCoincidences( [<index>, <attempts>] ) F

attempt, for the <i>th (or default) interactive {\ACE} process started
by `ACEStart', upto <attempts> (or, in the first and second forms,  8)
times to find nontrivial subgroups with index a multiple of <index> by
repeatedly making random coset numbers coincident  with  coset  1  and
seeing what happens. The starting coset table must be  non-empty,  but
need not be complete. For each attempt, we repeatedly add random coset
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\Section{Operations with ACE's Coset Table}

\beginitems

\>ACENormalClosure( <i> ) F
\>ACENormalClosure() F
\>ACENormalClosure( <i>, <add> ) F
\>ACENormalClosure( <add> ) F

for the <i>th (or  default)  interactive  {\ACE}  process  started  by
`ACEStart', if <add> (which *must* be boolean) is missing or  `false',
test for normal closure. If a subgroup generator does not normalise  a
generator, then  this  is  returned.  If  `<add>  =  true',  then  any
non-normalising conjugates are also added to the subgroup generators.

\>ACEOrders( <i> ) F
\>ACEOrders() F

return, for the <i>th (or default) interactive {\ACE} process  started
by `ACEStart', the list of orders (modulo the subgroup) of  all  coset
numbers.

\>ACEOrder( <i>, <int> ) F
\>ACEOrder( <int> ) F

search, for the <i>th (or default) interactive {\ACE} process  started
by `ACEStart', for cosets with order modulo the subgroup a multiple of
the absolute value of the integer <int>. If <int> is negative then the
list of all cosets meeting the requirement is returned.  If  <int>  is
positive then the first coset meeting the requirement is  returned  if
such a coset exists, and `fail' otherwise.

\>ACEStabilisingCosets( <i>, <int> ) F
\>ACEStabilisingCosets( <int> ) F

return, for the <i>th (or default) interactive {\ACE} process  started
by `ACEStart', stabilising cosets of the subgroup.

\beginlist
\item{--} If <int> $> 0$, the list  of  the  first  <int>  stabilising
cosets is returned. 

\item{--} If <int> $\< 0$, the  list  of  pairs  of  the  first  <int>
stabilising cosets, plus their representatives, is returned.

\item{--} If <int> = $0$, the list of pairs of all of the  stabilising
cosets, plus their representatives, is returned.
\endlist

\>ACETraceWord( <i>, <int>, <word> ) F
\>ACETraceWord( <int>, <word> ) F

trace, for the <i>th (or default) interactive {\ACE}  process  started
by `ACEStart', <word> through {\ACE}'s coset table, starting at  coset
<int>, and return the final coset number if the trace  completes,  and
`fail' otherwise.
