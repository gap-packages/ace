...
of the options provided here have interactive alternatives; a few have
only limited usefulness from {\GAP}; many, users  will  normally  only
wish to  use  if  generating  an  input  file,  by  using  the  option
`aceinfile' (see~"option aceinfile"). However  all  options  here  are
functional, both interactively and non-interactively.
...
property that elements that \lq{}normalise' a subgroup, may be said to
\lq{}stabilise' that subgroup  when  they  act  \lq{}by  conjugation'.
Also, the option `normal' (see~"option  normal")  already  performs  a
different function.
