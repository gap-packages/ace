...
call that invokes {\ACE}'s `print' option (see~"Option `print'")  will
do       this,       e.g.~`ACECosetTable'       (see~"ACECosetTable"),
`ACEDisplayCosetTable'                   (see~"ACEDisplayCosetTable"),
`ACECosetRepresentative' (see~"ACECosetRepresentative"). One can check
whether a function invokes {\ACE}'s  `print'  option,  by  trying  the
function after setting  the  `InfoLevel'  of  `InfoACE'  to  4.  Also,
`ACECycles'  (see~"ACECycles"),   among   others,   invokes   {\ACE}'s
compaction routine.
...
for the <i>th (or  default)  interactive  {\ACE}  process  started  by
`ACEStart', search for coset numbers with coset  representative  order
modulo the subgroup a multiple of the absolute value  of  the  integer
<int>. If <int> is negative then the list of all coset numbers meeting
the requirement is returned. If <int> is positive then the first coset
number meeting the requirement is returned  if  such  a  coset  number
...
return, for the <i>th (or default) interactive {\ACE} process  started
by `ACEStart', coset numbers of stabilising cosets of the subgroup.

\beginlist
\item{--} If <int> $> 0$, the list of the first <int> coset numbers of
stabilising cosets is returned.

\item{--} If <int> $\< 0$, the list of pairs of the first <int>  coset
numbers  of  stabilising  cosets,  plus  their   representatives,   is
returned.

\item{--} If <int> = $0$, the list  of  pairs  of  all  of  the  coset
numbers of stabilising cosets, plus their  coset  representatives,  is
returned. \endlist
