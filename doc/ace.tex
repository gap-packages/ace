...
{\undoquotes\atindex{ACECosetTable, break-loop example}
{@`ACECosetTable', `break'-loop example}}
\beginexample
...
Observe how the lines after the \lq{}`Entering  break  read-eval-print
...
standardisation  scheme}}  standardisation  to  occur  at  the  {\ACE}
...
\quad`activecosets'\index{activecosets}  &the  number   of   currently
*active* (i.e.~*alive*) coset numbers (see  Section~"Coset  Statistics
Terminology");

\quad`maxcosets'\index{maxcosets} &the *maximum* number of alive coset
numbers at  any  one  time  in  the  enumeration  (see  Section~"Coset
Statistics Terminology"); and

\quad`totcosets'\index{totcosets} &the *total* number of coset numbers
that were defined in the enumeration  (see  Section~"Coset  Statistics
Terminology").
...
\lq{}results message' (see  Appendix~"The  Meanings  of  ACE's  Output
Messages") from {\ACE}.

`ACEStats' may also be called in a different  way  in  an  interactive
{\ACE} session (see~"ACEStatsI").
