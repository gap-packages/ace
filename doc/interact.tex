...
\lq{}for the <i>th (or default) interactive {ACE} process'  a  certain
...
#I  ACE Version: 3.000
#I  Executable built:
#I    Tue Oct 17 10:45:14 EST 2000
#I  Level 0 options:
...
the presentation
$$\{ a, b \mid a^2, b^6, (ab^{-1}ab)^3, (ab^{-1}ab^2)^4,
               (ab)^5, (ab^-2ab^2)^2 \}$$ 
...
get a normalising element (we had warned above of such a possibility).
...
This time we got a normalising element, and after  adding  it  to  the
subgroup  generators  we  obtained  a  complete  table.  However   the
resulting subgroup need not yet be the normal closure of $H$  (and  in
fact we know that it is not). So we continue with another call to  the
function `ACEConjugatesForSubgroupNormalClosure'.
