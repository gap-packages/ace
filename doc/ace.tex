...
\endtt
\URL{http://www.csee.uq.edu.au/~havas/ace3.tar.gz}
...
\item{--} `ACEExample' now uses the `ACEStats'  function  by  default.
Originally `ACECosetTableFromGensAndRels' was used by default.

\atindex{CallACE (deprecated)}{@\noexpand`CallACE' (deprecated: use %
\noexpand`ACECosetTableFromGensAndRels')}
\item{--} Function   `CallACE'  (deprecated:  use  `ACECosetTable'  or
`ACECosetTableFromGensAndRels').

\atindex{option outfile (deprecated)}{@option \noexpand`outfile' %
(deprecated: use \noexpand`aceinfile')}
\item{--} Option `outfile' (deprecated: use `aceinfile').

\atindex{option messfile (deprecated)}{@option \noexpand`messfile' %
(deprecated: use \noexpand`aceoutfile')}
\item{--} Option `messfile' (deprecated: use `aceoutfile').

\atindex{ACEinfo (deprecated)}{@\noexpand`ACEinfo' (deprecated: old %
name for \noexpand`ACEData' record)}
\item{--} Data record `ACEinfo' (deprecated: old  name  for  `ACEData'
record). This change was made because there is now a new  `InfoClass':
`InfoACE' (and it was too confusing to have both it and `ACEinfo'!)
