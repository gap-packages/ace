...
(see Chapter~"Predefined Strategies"). An {\ACE} strategy is merely  a
special option of {\ACE} that sets a number of the  options  described
in Chapter~"Options for the ACE Enumerator" all at once. One  can  set
options   globally   using    the    function    `PushOptions'    (see
...
record components. Note that strategies  are  simply  special  options
that set a number of the options, detailed in Chapter~"Options for the
ACE Enumerator", all at once. These can be used e.g.~to preset  limits
of space and time to be used, to modify input and output and to modify
...
raises an error, unless the `silent'  option  (see~"parameter silent")
...
enumeration. A *dead* coset (number) is a coset found to be coincident
with a lower numbered coset. The default is 10 or  100,  depending  on
the strategy used (see Section~"Predefined Strategies").
...
coset numbers. For example, `compaction = 20'  means  compaction  will
occur only if 20\% or more of the cosets in the  table  are  dead.  An
argument of 100 means that compaction is  never  performed,  while  an
...
\lq{}good'. To help overcome this  problem,  {\ACE}  contains  various
...
options; order *is* important. What is special about strategy options,
is that they modify a whole swag of options defined  in  the  previous
chapter, all at once. So it is usual to specify  one  strategy  option
and possibly follow it  with  a  number  of  options  defined  in  the
previous chapter. Please refer to the  introduction  of  the  previous
...
the options (of Chapter~"Options for the ACE  Enumerator")  that  they
set, except that all  of  them  additionally  set  `path'  =  `false',
`psize' = $256$, and `dsize'  =  $1000$.  Recall  that  `mend'  is  an
abbreviation for `mendelsohn' (see~"parameter mendelsohn") and  `look'
is an abbreviation for `lookahead' (see~"parameter lookahead").
...
(see~"parameter asis"). These  parameters  have  an  effect  which  is
independant of the selected strategy.
...
Selects a Felsch strategy. (Shortest abbreviation: `fel'.)
...
subgroup and with gap-filling active), but that every  $1000$  C-style
definitions a further coset is applied to all  relators.
...
`lookahead' and no row-`fill'ing.

\>`sims:=<val>'{parameter sims}&
Set a Sims strategy; <val> should be one of 1, 3, 5, 7 or 9.

In his book~\cite{Sim94},  Sims  discusses  ten  standard  enumeration
strategies.  These  are  effectively  HLT  (with   and   without   the
`mendelsohn' parameter -see~"parameter mendelsohn")  and  Felsch,  all
either with  or  without  table  standardisation  as  the  enumeration
proceeds. {\ACE} does not implement table  standardisation  during  an
enumeration, although incomplete tables can  be  standardised  and  an
enumeration  continued.  The   five   odd-numbered   non-standardising
strategies of~\cite{Sim94} are implemented ({\ACE}'s numbering is  the
same as given  in  Section  5.5~of~\cite{Sim94}).  With  care,  it  is
possible to  duplicate  the  statistics  given  in~\cite{Sim94};  some
examples are given in Chapter~"Examples". 
...
and Ramsay  \cite{HR99b} such  equivalent presentations can  yield large
